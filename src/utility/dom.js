...
export function addEventListenerTouch(dom, type, callback) {
  if (typeof type !== 'string') {
    throw new TypeError('傳入的 type 參數必須是個 string')
  }
  if (typeof callback !== 'function') {
    throw new TypeError('傳入的 callback 參數必須是個 function')
  }
  ...
  if (this instanceof HTMLElement) {
    ...
    throw new TypeError('必須是個 HTMLElement')
  }
}
...
export const slideDown = function () {
  const [el, duration, easing, delay, callback] = paramFilter(arguments, [HTMLElement, Number, String, Number, Function])
...
/**
 * 取得絕對位置的box座標
 * @param {HTMLElement} target 要傳入的偵測DOM
 * @return {*} box = { x , y , path }
 */
export const getCoordinateBox = (target) => {
  const box = {
    y: 0,
    x: 0,
    path: [],
  ...
  function boxCount(dom) {
    ...
    if (offsetParent === document.body || offsetParent === document || offsetParent) {
      return document
    }
